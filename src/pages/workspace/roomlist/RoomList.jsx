...
const RoomList = ({ rooms = [], isLoading = false }) => {
  ...
  const filteredRooms = rooms.filter((room) => {
    ...
    const playersCount =
      room.players && Array.isArray(room.players) ? room.players.length : 0;
    return currentPlayer > 0 || playersCount > 0;
  });

  const roomList = filteredRooms.map(
    (
      {
        gameRoomCreateAt,
        gameRoomCurrentPlayer,
        gameRoomIsOpen,
        gameRoomIsStart,
        gameRoomIsTeam,
        gameRoomMaxPlayer,
      ...
      const handleEnter = () => {
        ...
        const hasPassword =
          gameRoomPassKey != null && String(gameRoomPassKey).trim() !== "";
        if (hasPassword) {
      ...
      const isLocked =
        (gameRoomPassKey != null && String(gameRoomPassKey).trim() !== "") ||
        !gameRoomIsOpen;
      const lockIconSrc = isLocked
        ? "/assets/gameroom/common/locker.png"
        : "/assets/gameroom/common/check.png";
      ...
      return (
        <S.RoomListPosition onClick={handleEnter} key={i}>
          <S.RoomList $isDisabled={isDisabled}>
            <S.RoomLeft>
              <img
                src="/assets/gameroom/common/group.svg"
                alt="flag"
                className="flag"></img>
              <S.RoomTitleWrapper>
                <p>{gameRoomTitle}</p>
            ...
            {isLocked ? (
              <img
                className="locker"
                src={"/assets/gameroom/common/locker.png"}
                alt="비공개방"
              />
            ) : (
              <></>
                ...
                <S.ProfileWrap>
                  {players.map((user) => {
                    ...
                    const isHost =
                      user.isHost === true ||
                      user.isHost === 1 ||
                      user.gameJoinIsHost === true ||
                      user.gameJoinIsHost === 1;
                    return (
                      <S.ProfileImgWrap key={user.id || user.userId}>
                        {isHost && (
                          <S.CrownIcon
                            src="/assets/gameroom/common/crown.png"
                            alt="host"
                          />
                        )}
                        <S.ProfileImg
                          src={`${process.env.REACT_APP_BACKEND_URL}/file/display?fileName=${user.userThumbnailURL}${user.userThumbnailName}`}
                          alt="userProfile"></S.ProfileImg>
                      </S.ProfileImgWrap>
                    );
                  })}
                </S.ProfileWrap>
                {averageLevel > 0 && (
                  <S.AverageLevelText $levelColor={getLevelColor(averageLevel)}>
                    Lv.{averageLevel}
                  </S.AverageLevelText>
                )}
              </S.ProfileWrapper>
          ...
          <S.PasswordHidden className="password">
            {gameRoomPassKey}
          </S.PasswordHidden>
        </S.RoomListPosition>
      );
  ...
  return (
    <S.RoomListWrap>
      <S.RoomListContainer>{roomList}</S.RoomListContainer>
    </S.RoomListWrap>
  );
};
