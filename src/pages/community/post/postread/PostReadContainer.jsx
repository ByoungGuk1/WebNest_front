...
const PostReadContainer = () => {
  ...
  useEffect(() => {
    const fetchPosts = async () => {
      try {
        // NOTE: 기존 경로 유지. 필요하면 '/json_server/post/post.json'로 변경
  ...
  if (!posts) return <S.LoadingMsg>게시글을 불러오는 중...</S.LoadingMsg>;
  if (!currentPost)
      ...
      <S.ContentWrap>
        <S.PostWrap>
          ...
          <S.PosterInfo>
            <S.LeftBox>
              <S.ProfileImg
                src={author?.profileImg || "/assets/images/defaultpro.svg"}
                alt={author?.name || "익명"}
              />
